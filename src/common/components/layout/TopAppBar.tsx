    ...
    <header className="sticky top-0 z-40 border-b border-[#e2dfde] bg-white">
      <div className="mx-auto grid h-16 max-w-[1440px] grid-cols-3 items-center px-4 md:px-10">
          ...
          {navItems.map((item) => (
            <a
              ...
              className={`text-[12px] font-semibold tracking-[0.05em] transition-colors ${
                item.active ? 'border-b-2 border-[#b90014] pb-1 text-[#b90014]' : 'text-[#5f5e5e] hover:text-[#b90014]'
              }`}
              href={item.href}
            >
              {item.label}
        ...
        <div className="justify-self-center">
          <Link className="text-[24px] font-extrabold tracking-[-0.02em] text-[#b90014] md:text-[30px]" to="/">
            RecruitPro
          </Link>
        </div>
