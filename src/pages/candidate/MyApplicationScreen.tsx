    ...
    <div className="min-h-screen bg-[#f9f9f9] text-[#1a1c1c]">
        ...
        <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between gap-4 px-4 md:px-10">
          <div className="flex items-center gap-8">
            ...
            <nav className="hidden items-center gap-6 md:flex">
              ...
              <Link className="border-b-2 border-[#b90014] pb-1 text-[12px] font-semibold tracking-[0.05em] text-[#b90014]" to="/candidate/my-applications">
                My Applications
              </Link>
            </nav>
          </div>
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            />
            <button type="button" className="text-[#5f5e5e] transition-colors hover:text-[#b90014]">
      ...
      <main className="mx-auto w-full max-w-[1440px] px-4 py-6 md:px-10 md:py-10 pb-24 lg:pb-10">
        <div className="mb-10 flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
          <div>
            <h1 className="mb-2 text-[32px] font-semibold leading-10 tracking-[-0.01em]">My Applications</h1>
            <p className="text-[14px] leading-5 text-[#5f5e5e]">
              Manage and track your active recruitment journeys across the RecruitPro network.
            </p>
          </div>

          <div className="grid grid-cols-3 gap-4">
            {summaryCards.map((card) => (
              <div key={card.label} className="min-w-[120px] border border-[#e2dfde] bg-white p-4 text-center">
                <span className="block text-[32px] font-bold text-[#b90014]">{card.value}</span>
        ...
        <div className="mb-8 flex flex-wrap items-center gap-4 bg-[#f3f3f3] p-4">
          <div className="flex items-center gap-2">
            <span className="text-[12px] font-semibold uppercase tracking-[0.05em] text-[#1a1c1c]">Filter By:</span>
              ...
              >
                <div className="flex items-start gap-4 lg:col-span-4">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center border border-[#e2dfde] bg-white">
                    <span className="material-symbols-outlined text-[#b90014]">{item.icon}</span>
                  </div>
                  <div>
                    <h3 className="text-[20px] font-semibold leading-7 group-hover:text-[#b90014]">{item.title}</h3>
                    <p className="text-[14px] leading-5 text-[#5f5e5e]">{item.department}</p>
                  </div>
                </div>

                <div className="text-[14px] text-[#5f5e5e] lg:col-span-2">
                  <span className="lg:hidden font-bold text-[#1a1c1c]">Applied: </span>
                  {item.appliedDate}
                </div>

                <div className="lg:col-span-2">
                  <span className={`inline-flex px-3 py-1 text-[10px] font-bold uppercase tracking-[0.05em] ${item.statusClass}`}>
                    {item.status}
                  </span>
                </div>

                <div className="text-[14px] italic text-[#5f5e5e] lg:col-span-2">{item.nextStep}</div>
                ...
                <div className="flex justify-end gap-3 lg:col-span-2">
                  ...
                  <button
                    ...
                    type="button"
                  >
                    {item.actionLabel === 'Accept Offer' ? 'Withdraw' : item.actionLabel}
                  </button>
                </div>
              </div>
            ))}
        ...
        <div className="mt-8 flex flex-col items-start justify-between gap-4 px-1 md:flex-row md:items-center">
          ...
          <div className="flex gap-2">
            <button className="flex h-10 w-10 items-center justify-center border border-[#e2dfde] transition-colors hover:bg-[#f3f3f3]" type="button">
              <span className="material-symbols-outlined">chevron_left</span>
            </button>
            <button className="flex h-10 w-10 items-center justify-center border border-[#1a1c1c] bg-[#1a1c1c] font-bold text-white" type="button">
              1
            </button>
            <button className="flex h-10 w-10 items-center justify-center border border-[#e2dfde] transition-colors hover:bg-[#f3f3f3]" type="button">
              2
