...
function CandidateProfileAndCVManagementScreen() {
  ...
  function handleSaveProfile() {
    console.log('[mock-json]', profilePayload)
    ...
    setExperienceEntries((prev) => {
      const updated = newEntry.period.isCurrent
        ? prev.map((entry, index) => {
            const currentIndex = prev.findIndex((item) => item.period.isCurrent)

            if (index !== currentIndex || currentIndex === -1 || !entry.period.isCurrent) return entry
      ...
      <header className="sticky top-0 z-40 border-b border-[#e2dfde] bg-white">
        <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between gap-4 px-4 md:px-10">
          <div className="flex items-center gap-8">
            <Link to="/candidate" className="text-[24px] font-extrabold tracking-[-0.02em] text-[#b90014]">
              RecruitPro
            </Link>
            <nav className="hidden items-center gap-6 md:flex">
              <Link className="text-[12px] font-semibold tracking-[0.05em] text-[#5f5e5e] hover:text-[#b90014]" to="/candidate">
                Home
              </Link>
              <Link className="text-[12px] font-semibold tracking-[0.05em] text-[#5f5e5e] hover:text-[#b90014]" to="/candidate/my-applications">
                My Applications
              </Link>
              <Link className="border-b-2 border-[#b90014] pb-1 text-[12px] font-semibold tracking-[0.05em] text-[#b90014]" to="/candidate/profile">
                Profile
              </Link>
            </nav>
          </div>
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <CandidateAvatarMenu
              ...
              avatarSrc="https://lh3.googleusercontent.com/aida-public/AB6AXuAv0v_JFlncGaMzvhobx8TVjvDkl_FfcTYrRB24lXEVSbqrUcKF-b9Zv7V10Paw7EP9TNTuojK3OfXwgF9AqUzfmfYUh8XEy2k2HkRll4OlDeareuUbvHLdEhv-nB_7v9NBPzxv3sx-UEftIPP13d3DGt_lPmwwHhq1f-kQ3FE33lLLMfIYK0Omw4_VJLr3nld1uLamLUQ9ADC31kLw0p_OpTSXHPVvcC4d4YY5AWwQgX3q_DfImyhtiS57pWXGxH0_mT_bvLsn2g"
              items={[
                { label: 'My Applications', to: '/candidate/my-applications' },
                { label: 'Manage CV', to: '/candidate/profile#resume' },
              ]}
              logoutTo="/candidate/login"
            />
          </div>
        </div>
      </header>

      <main className="mx-auto w-full max-w-[1440px] px-4 py-6 md:px-10 md:py-10">
        <nav className="mb-6 flex items-center gap-2 text-[12px] font-semibold uppercase tracking-[0.05em] text-[#5f5e5e]">
          <Link className="transition-colors hover:text-[#b90014]" to="/candidate/my-applications">
            Applications
          </Link>
          <span className="material-symbols-outlined text-[16px]">chevron_right</span>
          <span className="text-[#1a1c1c]">Candidate Profile</span>
        </nav>

        <section id="profile" className="relative mb-6 overflow-hidden rounded-xl border border-[#e2dfde] bg-white p-6 md:p-8">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {isEditingProfile ? (
                    ...
                    <p className="mt-1 text-[20px] font-semibold text-[#b90014]">{profile.headline}</p>
                  )}
                </div>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
            <div className="space-y-6 lg:col-span-4">
              <section id="resume" className="rounded-lg border border-[#e2dfde] bg-white p-6">
                <h2 className="border-l-4 border-[#b90014] pl-4 text-[20px] font-semibold">Personal Information</h2>
              ...
              <section className="rounded-lg border border-[#e2dfde] bg-white p-6">
                <div className="mb-6 flex items-center justify-between gap-3">
                  <h2 className="border-l-4 border-[#b90014] pl-4 text-[20px] font-semibold">Skills</h2>
                  <button
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  <div className="flex items-center gap-4 rounded border border-[#e2dfde] bg-[#f3f3f3] p-4 transition-colors hover:border-[#b90014]">
                    <div className="flex h-12 w-12 items-center justify-center rounded bg-[#b90014]/10">
                      <span className="material-symbols-outlined text-[32px] text-[#b90014]">picture_as_pdf</span>
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[16px] font-semibold">Alex_Thompson_CV_2024.pdf</p>
                      <p className="text-[12px] font-semibold uppercase tracking-[0.05em] text-[#5f5e5e]">
                        Uploaded on Oct 12, 2024
                      </p>
                    </div>
                    <div className="flex gap-1">
                      <button className="p-2 text-[#5f5e5e] transition-colors hover:text-[#b90014]" type="button">
                  ...
                  <button
                    className="flex min-h-[104px] flex-col items-center justify-center rounded border-2 border-dashed border-[#e2dfde] bg-[#f3f3f3] text-[#5f5e5e] transition-colors hover:border-[#b90014] hover:text-[#1a1c1c]"
                    type="button"
                    onClick={() => console.log('[mock-json]', profilePayload)}
                  >
                  ...
                  <div className="mb-8 space-y-3 rounded border border-[#e2dfde] bg-[#f3f3f3] p-4">
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                      ...
                      <input
                        ...
                        onChange={(e) => setEntryDraft((prev) => ({ ...prev, period: e.target.value }))}
                      />
                      <input
                        className="rounded-none border border-[#e2dfde] bg-white px-3 py-2 text-[14px] outline-none focus:border-[#1a1c1c]"
                        placeholder="Company / School"
                        value={entryDraft.company}
                        onChange={(e) => setEntryDraft((prev) => ({ ...prev, company: e.target.value }))}
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                      <select
                        ...
                        onChange={(e) =>
                          setEntryDraft((prev) => ({ ...prev, startMonth: Number(e.target.value) }))
                        }
                      >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEntryDraft((prev) => ({ ...prev, startYear: Number(e.target.value) }))
                        }
                      />
                      <label className="col-span-2 flex items-center gap-2 rounded border border-[#e2dfde] bg-white px-3 py-2 text-[14px] font-semibold text-[#1a1c1c] md:col-span-1">
                        <input
                          ...
                          type="checkbox"
                          onChange={(e) =>
                            setEntryDraft((prev) => ({ ...prev, isCurrent: e.target.checked }))
                          }
                        />
                        Present
                      </label>
                      {entryDraft.isCurrent ? null : (
                        <>
                          <select
                            ...
                            onChange={(e) =>
                              setEntryDraft((prev) => ({ ...prev, endMonth: Number(e.target.value) }))
                            }
                          >
                            {monthOptions.map((month, index) => (
                              <option key={month} value={index + 1}>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEntryDraft((prev) => ({ ...prev, endYear: Number(e.target.value) }))
                            }
                          />
                        </>
                      )}
                    ...
                    <textarea
                      ...
                      onChange={(e) => setEntryDraft((prev) => ({ ...prev, bullets: e.target.value }))}
                    />
